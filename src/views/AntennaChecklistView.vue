...
function handleDownloadTemplate() {
  downloadAntennaChecklistTemplate(customColumns.value)
...
async function handleExport() {
  ...
  downloadAntennaChecklistExport(rows.value, site.value?.name || siteId, customColumns.value)
...
async function handleImportFile(event) {
  ...
  try {
    const parsed = await parseAntennaChecklistSpreadsheet(file)
...
async function saveDraftField(row, field) {
  ...
  const value = field === 'fieldValues' ? draft.fieldValues || {} : draft[field] || ''
...
async function handleAddColumn() {
  ...
  try {
    ...
    await logAction('Antenna checklist column added', `${siteId} - ${label}`)
    showStatus('Antenna checklist column added.')
    closeAddColumnModal()
  } catch (error) {
...
async function handleRemoveColumn(column) {
  ...
  await logAction('Antenna checklist column removed', `${siteId} - ${column.label}`)
  showStatus('Antenna checklist column removed.')
}
      ...
      >
        <div class="add-site-modal box col gap-4">
          <div class="between" style="align-items: center">
            <div class="title-md">Add antenna checklist column</div>
          ...
          <div class="col gap-2">
            ...
            <input
              ...
              class="field"
              type="text"
              placeholder="Example: Mount type"
          ...
          <div class="col gap-2">
            ...
            <div v-if="!customColumns.length" class="box-soft p-3 small" style="color: var(--ink)">
              No added columns yet. Baseline antenna checklist columns stay in place and cannot be removed here.
            </div>
